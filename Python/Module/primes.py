...
count_each_char_sort_value = lambda string:sorted(list(set([(c, string.count(c))for i, c in enumerate(string)])), key = lambda n:n[0])
...
def prime_factorization(n, mode = "single"):
	if not isPositiveInterger([n]):
		raise AttributeError("Input not valid")
	if n == 0:
		return [(0, 1)]
	if miller_rabin(n, 3):
		return [n] if mode == "single" else [(n, 1)]
	result = []
	i = 2
	while n % 2 == 0:
		result.append(2)
